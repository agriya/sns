...
/**
 * @ngdoc function
 * @name Sns.controller:designerController
 * @description
 * #designer Controller
 * Controller of the Sns
 */
angular.module('SnsApp')
    .controller('designerController', ['$scope', '$rootScope', '$location', '$window', '$filter', 'md5', 'designerFactory', 'UserUnfollowFactory', 'UserFollowFactory', '$uibModal', 'FlagCategoryFactory', '$uibModalStack', '$state', 'UserPhotosFactory', 'FlickityService', function($scope, $rootScope, $location, $window, $filter, md5, designerFactory, UserUnfollowFactory, UserFollowFactory, $uibModal, FlagCategoryFactory, $uibModalStack, $state, UserPhotosFactory, FlickityService) {
        $scope.enabled = true;
        $scope.users = [];
        $scope.follow = [];
        $scope.user_photo = [];
        ...
        $scope.init = function() {
            ...
            $rootScope.header = $rootScope.settings.SITE_NAME + ' | ' + $filter("translate")("Designer Popular");
        ...
        $scope.getUserPhoto = function() {
            ...
            designerFactory.get(params, function(response) {
                $scope.scroll_flag = true;
        ...
        $scope.go = function(user_id, user_name) {
            $location.path('profile/' + user_id + '/' + user_name);
        };
        //init triggered
        $scope.init();
        //photo model
